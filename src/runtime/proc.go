...
func execute(gp *g, inheritTime bool) {
	...
	if _g_.m.profConfig[CPUPROF_OS_TIMER] != sched.profConfig[CPUPROF_OS_TIMER] {
		setThreadOSTimerProfiler(sched.profConfig[CPUPROF_OS_TIMER])
	}

	// Check whether the PMU profilers need to be turned on or off.
	if setThreadPMUProfilerFunc != nil {
		profConfig := sched.profConfig
		for eventId := CPUPROF_FIRST_PMU_EVENT; eventId < CPUPROF_EVENTS_MAX; eventId++ {
...
var prof [CPUPROF_EVENTS_MAX]struct {
...
var lostAtomic64Count [CPUPROF_EVENTS_MAX]uint64
...
func setcpuprofileconfig(eventId cpuEvent, profConfig *cpuProfileConfig) {
	...
	if (eventId != CPUPROF_OS_TIMER) && (setThreadPMUProfilerFunc == nil) {
		return
	}
