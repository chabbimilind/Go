...
func StartPMUProfile(opts ...ProfilingOption) error {
	if runtime.GOOS != "linux" || (runtime.GOARCH != "amd64" && runtime.GOARCH != "arm64") { // enabling only on Linux AMD64/ARM64
		return fmt.Errorf("StartPMUProfile not implemented on %s/%s", runtime.GOOS, runtime.GOARCH)
	}
	mux.Lock()
	defer mux.Unlock()
