...
func stdcall(fn stdFunction) uintptr {
	...
	resetLibcall := false
	if mp.profConfig[CPUPROF_OS_TIMER] != nil && mp.profConfig[CPUPROF_OS_TIMER].hz != 0 && mp.libcallsp == 0 {
...
func profilem(mp *m, thread uintptr) {
	...
	sigprof(r.ip(), r.sp(), r.lr(), gp, mp, CPUPROF_OS_TIMER)
}
...
func profileloop1(param uintptr) uint32 {
	...
	for {
		...
		for mp := first; mp != nil; mp = mp.alllink {
			...
			if mp.thread == 0 || mp.profConfig[CPUPROF_OS_TIMER] == nil || mp.profConfig[CPUPROF_OS_TIMER].hz == 0 || mp.blocked {
				unlock(&mp.threadLock)
				continue
			}
			...
			if mp.profConfig[CPUPROF_OS_TIMER] != nil && mp.profConfig[CPUPROF_OS_TIMER].hz != 0 && !mp.blocked {
...
func setThreadOSTimerProfiler(profConfig *cpuProfileConfig) {
	...
	atomic.Storeuintptr((*uintptr)(unsafe.Pointer(&getg().m.profConfig[CPUPROF_OS_TIMER])), uintptr(unsafe.Pointer(profConfig)))
}
